...
- `impl/js/src/` の著者(TypeScript リファレンス実装)
- `spec/` の編集者(XTL 言語の定義)
- `spec/decisions/` における [ADR](https://github.com/xl3-lang/xl3/tree/main/spec/decisions) の承認者
- すべての PR のレビュアー
...
| **ポート著者** | XTL を別の言語で実装し、[`conformance/fixtures/`](https://github.com/xl3-lang/xl3/tree/main/conformance/fixtures) に対して適合性を測定する。 | 誰でも可。[IMPLEMENTATIONS.md](/implementations) に掲載されます。 |
| **コントリビューター** | イシューを立て、PR を送り、フィクスチャや ADR を提案する。 | 誰でも可。[CONTRIBUTING.md](/contributing) を参照。 |
...
3. **ADR ドラフト** — メンテナ(またはコントリビューター)が `spec/decisions/0000-template.md` のテンプレートに従って [`spec/decisions/`](https://github.com/xl3-lang/xl3/tree/main/spec/decisions) に ADR を起草する。Context、Considered Options、Decision、Consequences、References を含める。
...
[`spec/decisions/`](https://github.com/xl3-lang/xl3/tree/main/spec/decisions) の ADR 群は、規範的な決定とその背後にある推論をすべて記録するプロジェクトの公開アーカイブです。
...
2. [`conformance/AUTHORING.md`](/conformance/authoring) に沿ってフィクスチャを作成する。基本ルール: **期待出力は、リファレンス実装を実行した結果ではなく、仕様から導く。**
3. PR を送る。
...
| `xl3` npm `1.x`(リリース後) | 公開 API は `impl/js/src/__tests__/api-surface.test.ts` のスナップショットで凍結。改名や削除はメジャー更新を要する |
...
3. **別言語に移植して適合性コーパスを実行する。** 独立した第二実装はどんなレビューよりも素早く仕様のギャップを見つけます。[PORTERS_GUIDE.md](https://xl3.io/porters-guide) を参照。
4. **先送り項目について ADR ドラフトを開く。** [`spec/decisions/`](https://github.com/xl3-lang/xl3/tree/main/spec/decisions) で先送りされた項目(日付演算、ロケール照合、マルチ結合等)は将来の ADR 候補です — 具体的な提案を歓迎します。
