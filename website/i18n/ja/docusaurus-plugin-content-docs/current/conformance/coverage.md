...
| 0004 | Reference implementation coupling audit | _(参考情報)_ |
...
| 0034 | Relationship to prior-art template engines | _(参考情報)_ |
...
| 0037 | Rejected: dynamic image insertion | _(参考情報)_ |
| 0038 | `@group` and `@subtotal` directives | `132-group-single-level-subtotal`, `133-group-two-level-nested-subtotal`, `134-group-grand-total-via-outermost-subtotal`, `135-group-filter-composition`, `136-group-missing-key`, `137-subtotal-outside-group`, `138-subtotal-bad-aggregate` |
| 0039 | HYPERLINK() function | `125-hyperlink-function` |
| 0040 | Preservation matrix amendment: CF / DV range extension + outline level | _(参考情報)_ |
| 0041 | Multi-line cell text | `127-multiline-cell-text` |
| 0042 | Rejected: runtime cell mutation (`jx:updateCell` style) | _(参考情報)_ |
| 0043 | Excel-native preference principle | _(参考情報)_ |
| 0044 | Function batch — accepted per ADR-0043 | `128-function-batch-0044` |
| 0045 | Function batch — rejected per ADR-0043 | _(参考情報)_ |
...
| 0048 | Relationship to JXLS — final boundary and inconvenience refinement | _(参考情報)_ |
| 0049 | Template-display vs render-output: intentional asymmetry | _(参考情報)_ |
...
| 0051 | ADR 0051 — String literals and template-block delimiter boundary | _(参考情報)_ |
| 0052 | ADR 0052 — Cell expression classification: single vs mixed text | _(参考情報)_ |
| 0053 | ADR 0053 — Mixed-text propagation of Excel error sentinels | _(参考情報)_ |
| 0054 | ADR 0054 — Bare name in cell context | _(参考情報)_ |
| 0055 | ADR 0055 — Directive integer arguments: positive-integer bounds | _(参考情報)_ |
| 0056 | ADR 0056 — Reading reserved-sheet system keys | _(参考情報)_ |
| 0057 | ADR 0057 — `__lists__[name]` outside `@filter in` / `!in` | _(参考情報)_ |
| 0058 | ADR 0058 — `@subtotal` row composition: same-row level binding | _(参考情報)_ |
| 0059 | ADR 0059 — Aggregate function argument shape | _(参考情報)_ |
| 0060 | ADR 0060 — `XLOOKUP` value-argument cross-source rules | _(参考情報)_ |
| 0061 | ADR 0061 — Source name lexical disambiguation | _(参考情報)_ |
| 0062 | ADR 0062 — `__inputs__` `default = ""` semantics | _(参考情報)_ |
| 0063 | ADR 0063 — `__inputs__` `options` pipe-split rules | _(参考情報)_ |
| 0064 | ADR 0064 — String-to-number coercion: scientific-notation scope | _(参考情報)_ |
| 0065 | ADR 0065 — `@source default` explicit reference | _(参考情報)_ |
