---
sidebar_label: '05 · 그룹별로 시트 나누기'
pagination_label: '05 · 그룹별로 시트 나누기'
---
...
한 블록 안의 `@filter` 디렉티브 여러 개는 AND로 합성됩니다. 즉 각 필터가 직전 결과를 더 좁혀 들어갑니다.

## 참고

- 시트 이름은 Excel의 31자 제한과 금지 문자(`[ ] / \ ? *`)에 맞춰 정제됩니다. 정제 결과가 충돌하는 경우의 동작은 ADR-0021에 따라 구현이 정의합니다 — 그룹 키 자체를 상류에서 유일하게 유지하세요.
- 빈 그룹 키는 ADR-0026에 따라 `(blank)` 리터럴로 처리됩니다.
- 시트 순서는 ADR-0016에 따라 처음 등장한 순서를 따릅니다.
