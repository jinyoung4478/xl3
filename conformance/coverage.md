...
Auto-generated by `impl/js/src/__tests__/spec-coverage.test.ts`. Do not edit manually.
...
| 0038 | `@group` and `@subtotal` directives | `132-group-single-level-subtotal`, `133-group-two-level-nested-subtotal`, `134-group-grand-total-via-outermost-subtotal`, `135-group-filter-composition`, `136-group-missing-key`, `137-subtotal-outside-group`, `138-subtotal-bad-aggregate`, `157-group-block-side-cells`, `159-subtotal-mixed-row-error`, `160-subtotal-formula-cache-not-marker`, `161-explicit-block-subtotal-unsupported` |
...
| 0046 | Cell formula preservation contract | `129-cell-formula-preservation`, `160-subtotal-formula-cache-not-marker` |
...
| 0058 | ADR 0058 — `@subtotal` row composition: same-row level binding | `159-subtotal-mixed-row-error` |
...
| 0068 | ADR 0068 — Multi-block detection (strict mode) | `147-multi-block-different-sources`, `148-multi-block-different-start-rows`, `151-block-overlap-error`, `161-explicit-block-subtotal-unsupported` |
...
| 0073 | ADR 0073 — `@subtotal` mixed-row is an error; a formula cell's cached result is not template text (amends ADR-0038/0058, ADR-0046) | `159-subtotal-mixed-row-error`, `160-subtotal-formula-cache-not-marker` |
| 0074 | ADR 0074 — group + subtotal is unsupported in explicit `@block` mode (amends ADR-0038) | `161-explicit-block-subtotal-unsupported` |
| 0075 | ADR 0075 — `xl3-source-json`: a language-neutral JSON source input format | _(informational)_ |
